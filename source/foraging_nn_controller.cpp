...
CForagingNNController::SFoodData::SFoodData() :
   HasFoodItem(false),
   FoodItemIdx(0),
   TotalFoodItems(0) {}

void CForagingNNController::SFoodData::Reset() {
   HasFoodItem = false;
   FoodItemIdx = 0;
   TotalFoodItems = 0;
}
...
CForagingNNController::SDiffusionParams::SDiffusionParams() :
   GoStraightAngleRange(CRadians(-1.0f), CRadians(1.0f)) {}

void CForagingNNController::SDiffusionParams::Init(TConfigurationNode& t_node) {
   try {
...
void CForagingNNController::SWheelTurningParams::Init(TConfigurationNode& t_node) {
   try {
...
CForagingNNController::SStateData::SStateData() :
   ProbRange(0.0f, 1.0f) {}
...
void CForagingNNController::Init(TConfigurationNode& t_node) {
   try {
      /*
...
void CForagingNNController::Reset() {
   /* Reset robot state */
   m_sStateData.Reset();
   /* Reset food data */
   m_sFoodData.Reset();
...
CVector2 CForagingNNController::CalculateVectorToLight() {
   ...
   if(cAccumulator.Length() > 0.0f) {
      if(m_sStateData.State == SStateData::STATE_EXPLORING) {
         return CVector2(0.1f, cAccumulator.Angle());
      }
      else {
...
CVector2 CForagingNNController::DiffusionVector(bool& b_collision) {
...
void CForagingNNController::SetWheelSpeedsFromVector(const CVector2& c_heading) {
...
void CForagingNNController::Explore() {
   ...
   if(bReturnToNest) {
      ...
      if(m_sStateData.InNest) {
         ...
         SetWheelSpeedsFromVector(
            ...
            m_sWheelTurningParams.MaxSpeed * 0.5f * CalculateVectorToLight());
      }
      else {
 ...
 */
REGISTER_CONTROLLER(CForagingNNController, "foraging_nn_controller")
